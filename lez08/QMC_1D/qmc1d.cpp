...
double variationalWaveFunction(double v)
{
	...
	//return exp(-0.5*v*v);
	double mu=0.801502;
	double sigma=0.625213;
	return exp( -((v-mu)*(v-mu))/(2.0*sigma*sigma) ) + exp( -((v+mu)*(v+mu))/(2.0*sigma*sigma) );
}
...
double variationalWaveFunction_second(double v)
{
	...
	double mu=0.801502;
	double sigma=0.625213;
	return (1/(sigma*sigma))*( -variationalWaveFunction(v)+(((v-mu)*(v-mu))/(sigma*sigma))*exp(-((v-mu)*(v-mu))/(2*sigma*sigma)) + (((v+mu)*(v+mu))/(sigma*sigma))*exp(-((v+mu)*(v+mu))/(2.0*sigma*sigma)) );
}
