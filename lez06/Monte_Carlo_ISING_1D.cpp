...
void Move(int metro)
{
  ...
  for(int i=0; i<nspin; ++i)
  {
  //Select randomly a particle ( 0 <= o <= nspin-1)
...
// INCLUDE YOUR CODE HERE
    	...
    	energy_up=Ising(+1, o, s);
      energy_down=Ising(-1, o, s);
			p=1./(1.+exp(-(energy_down-energy_up)/temp)); //prob spin up
			sm=rnd.Rannyu();
      if(sm<=p) s[o]=+1;
      else s[o]=-1;
      accepted++;
...
//contributo energetico dovuto allo spin sm in posizione ip
...
double Ising(int sign, int pos, double *s){
  double prod=0, som=0;
  s[pos]=sign;
  for(int i=0; i<nspin; ++i){
    prod+=s[i]*s[Pbc(i+1)];
...
//cycle over spins
  ...
  double chi=m*m;
  walker[iu] = u;
  walker[ic] = u2; //H**2! C(T)= <H*H>-<H>**2
  walker[im] = m;
  walker[ix] = chi; //m**2: chi=<m*m>-<m>**2
...
// INCLUDE YOUR CODE HERE 
   ...
   const int wd=20;
    ...
    Chi.open("output.chi.0",ios::app);
		//stima_x = beta*(blk_av[ix]/blk_norm/(double)nspin-stima_m*stima_m*nspin); //chi
		stima_x = beta*(blk_av[ix]/blk_norm/(double)nspin); //chi con h=0
		glob_av[ix]  += stima_x;
    ...
    Chi.close();
		...
		string f1,f2;
		if(iblk==nblk && restart==1){
			if(metro==1) {f1="Metropolis";}
			else {f1="Gibbs";}
			if(h==0) {f2="h0";}
			else {f2="h";}
    	EneT.open("../lez06/"+f1+"/eneT_"+f2+".res",ios::app);
    	HeatT.open("../lez06/"+f1+"/heatT_"+f2+".res",ios::app);
    	ChiT.open("../lez06/"+f1+"/ChiT_"+f2+".res",ios::app);
   		MagT.open("../lez06/"+f1+"/MagT_"+f2+".res",ios::app);
